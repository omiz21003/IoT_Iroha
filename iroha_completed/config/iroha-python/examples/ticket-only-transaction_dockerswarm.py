...
if __name__ == '__main__':
    try:
        s = ''.join(random.choices('strings.ascii_lowercase', k=20))
        create_ticket(domain='CR', tag_id= a_id+s)
        add_coin_to_admin(tag_id=a_id+s+'#CR')
        transfer_coin('admin@test', 'sfn1@CR', a_id+s+'#CR')
        get_coin_info(tag_id=a_id+s+'#CR')
